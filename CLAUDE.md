...
**Phase 2**（2026-06-09 ~ 06-14）— [retro](my-ai-platform/docs/retro/phase2-retro.md) ✅ 9/10 项完成
...
- ✅ 模型按场景分流：`resolve_model(agent_id)` + GPT/Gemini provider（本 commit）
...
4. **范围控制**：Phase 3 锚点 — verdict-detect / A2A evals / MultiMentionOrchestrator / Gemini
...
✅ 已写完 — 见 [retro](my-ai-platform/docs/retro/phase2-retro.md)。9/10 项完成，唯一遗留：verdict-detect。

## 下一步（优先级排序，Phase 3 锚点）

1. **verdict-detect** 🟡 — 链路终止判定，防止 Agent 互踢皮球。当前 `recursion_limit=10` 是硬兜底
2. **A2A evals 升级** 📋 — Phase 1 黄金集只测单 Agent tool call，需要 A2A 端到端 evals（knowledge → review 链路至少 5 条）
3. **MultiMentionOrchestrator** 🟡 — 多 mention 并行执行，当前只串行取第一个
4. **Gemini 接入** 🟡 — 联想/头脑风暴场景，前置条件：黄金集人评 ≥5 条"需要联想视角"的样本
5. **a2a-shadow-detection** 🟡 — 行内 @mention 扫描，当前只扫行首
