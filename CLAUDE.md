...
1. **A2A evals 升级** 📋 — Phase 1 黄金集只测单 Agent tool call，需要 A2A 端到端 evals（knowledge → review 链路至少 5 条）
2. **MultiMentionOrchestrator** 🟡 — 多 mention 并行执行，当前只串行取第一个
3. **Gemini 接入** 🟡 — 联想/头脑风暴场景，前置条件：黄金集人评 ≥5 条"需要联想视角"的样本
4. **a2a-shadow-detection** 🟡 — 行内 @mention 扫描，当前只扫行首
