...
│      ▼                                                 │
│   ┌─────────────────────────────────────────────┐      │
│   │     BudgetGuard（per-session / per-day）     │      │
│   └─────────────────────┬───────────────────────┘      │
│                         ▼                              │
│   ┌─────────────────────────────────────────────┐      │
│   │   MultiCatGraph（LangGraph StateGraph）     │      │
│   │   外层：Prompt-Chained Handoff              │      │
│   │                                             │      │
│   │   __start__ → [claude] → routeToNextCat     │      │
│   │                 ↑               │           │      │
│   │                 └── [gpt] ◄─────┘           │      │
│   │                        └── [gemini] ◄──┘   │      │
│   │   parseA2AMentions()：自己写的 9 条规则     │      │
│   │   depth 上限 MAX_A2A_DEPTH = 15             │      │
│   └─────────────────────┬───────────────────────┘      │
│                         │ 每个节点调用↓                 │
│                         ▼                              │
│   ┌─────────────────────────────────────────────┐      │
│   │   ToolCallerGraph（LangGraph StateGraph）   │      │
│   │   内层：ReAct Tool Loop                     │      │
│   │                                             │      │
│   │   call_model → shouldContinue?              │      │
│   │       │ 有 tool_use                         │      │
│   │       ▼                                     │      │
│   │   run_tools（ToolNode）→ 回到 call_model    │      │
│   │       │ 无 tool_use                         │      │
│   │       ▼                                     │      │
│   │     END（返回 finalText）                   │      │
│   │   recursionLimit = MAX_TOOL_LOOP_ITERATIONS │      │
│   └─────────────────────┬───────────────────────┘      │
│                         │                              │
│              ┌──────────┼──────────┐                   │
│              ▼          ▼          ▼                   │
│         ChatAnthropic  ChatOpenAI  ChatGoogleGenAI     │
│         （LangChain provider 层，统一 .stream()）      │
└────────────────────────────────────────────────────────┘
                           │
              ┌────────────┼────────────┐
              ▼            ▼            ▼
...
> **看图记两件事：**
> 1. **内层 ToolCallerGraph** — 模型自己决定调不调工具，这是 Agent 自主性的核心，LangGraph 负责图跳转。
> 2. **外层 MultiCatGraph** — `parseA2AMentions` 是我自己写的字符串扫描，LangGraph 按返回值决定跳哪个节点，这不是模型自主。
...
#### 3.3.2 ToolCallerGraph：LangGraph 实现的 ReAct loop

这是 §5.2 里 `ToolCallerGraph` 的简版逻辑说明。完整代码见 §5.2。

核心映射关系：

```
手写 runToolLoop 里的结构    →    LangGraph 里的对应概念
─────────────────────────────────────────────────────
for (let iter = 0; ...)     →    recursionLimit（自动防死循环）
adapter.stream(messages)    →    call_model 节点（ChatAnthropic.invoke）
pendingToolUses.length > 0  →    shouldContinue 条件边
tool.run(input)             →    run_tools 节点（ToolNode 自动分发）
messages.push(tool_result)  →    MessagesAnnotation reducer（自动追加）
```

> **关于 `recursionLimit`**：Anthropic / OpenAI / Gemini 的官方 SDK 一次 `messages.create` 或 `stream` 调用本身**不做循环**。LangGraph 的 `recursionLimit`（默认 25，`.compile()` 或单次 `.invoke()` 都可以传）是循环的防爆盖——Phase 1 设 10 够用。
...
#### 3.4.2 MultiCatGraph：LangGraph 实现的 Prompt-Chained Handoff

这是 §5.2 里 `MultiCatGraph` 的逻辑说明。完整代码见 §5.2。

核心映射关系：

```
手写 routeSerial 里的结构         →    LangGraph 里的对应概念
──────────────────────────────────────────────────────────────
while queue 非空 && depth < 15  →    MultiCatGraph 图循环 + recursionLimit
queue.shift()（取下一只 cat）    →    当前激活节点（claude/gpt/gemini）
adapter.invoke(messages, tools)  →    invokeCat 节点（内部跑 ToolCallerGraph）
parseA2AMentions(finalText)      →    routeToNextCat 条件边（自己实现）
queue.push(...nextCats)          →    条件边返回下一节点名称
depth < MAX_A2A_DEPTH            →    depth 写入 MultiCatState，条件边判断
```

教学版 routeSerial（约 30 行，展示原理用，实际跑的是 LangGraph 版）：

```typescript
// 真实 Clowder route-serial.ts 是 2376 行——差出来的不是冗余，是兜底（见 §3.6）
async function routeSerial(initialMessage: string, ctx: RequestContext) {
  ...
  while (queue.length > 0 && depth < MAX_A2A_DEPTH) {
    ...
    // 单 Cat 一回合 = 一整轮 ReAct Tool Loop（§3.3 / ToolCallerGraph）
    let finalText = ''
    for await (const chunk of toolCallerGraph.stream({ messages }, config)) {
      if (chunk.type === 'text') finalText += chunk.delta
      ctx.stream.send(catId, chunk)
    }
    ctx.history.push({ catId, content: finalText })

    // Prompt-Chained Handoff：自己写的 9 条规则
...
### 3.7 框架边界：LangGraph + LangChain 管什么，我自己写什么

> **本项目选择使用 LangGraph.js + LangChain.js**，原因：已有框架基础、LangGraph 的 StateGraph 可视化是学习目标之一、
> 框架的图结构让状态流转更容易调试。下面明确每一层的职责划分，防止"框架管太多 → 原理变黑盒"。

**LangGraph 管：状态图结构（把两条主轴变成可见的图）**

```
内层 ReAct Tool Loop → 一个 ToolCallerGraph：
  节点：call_model → run_tools（条件边：有 tool_use 才跳）
  这就是 LangGraph 文档里的标准 ReAct agent，直接复用

外层 Prompt-Chained Orchestration → 一个 MultiCatGraph：
  节点：invoke_current_cat → parse_mentions（条件边：parseA2AMentions）
  条件边的路由逻辑（parseA2AMentions）完全自己写，LangGraph 只管图跳转
```

**LangChain 管：工具定义、Embeddings、向量检索**

```
@langchain/core/tools  →  tool() 定义 searchNotes / saveNote
@langchain/openai 或 @langchain/anthropic  →  ChatAnthropic / ChatOpenAI
MemorySaver（langgraph）  →  工作记忆的 checkpoint
OpenAIEmbeddings / AnthropicEmbeddings  →  Phase 2 向量生成
不用：AgentExecutor / LLMChain / Chain —— 黑盒太多，看不见 tool loop
```

**我自己写（框架替代不了的核心逻辑）：**

```
parseA2AMentions()   —— §3.4.3 的 9 条规则，框架没有这个
MultiCatGraph 的条件边函数 routeToNextCat()  ——  读 parseA2AMentions 的结果
assembleContext()    ——  三层记忆组装成 ChatMessage[]，MemorySaver 只管 checkpoint
数据库 CRUD          ——  SQLite 的 notes / messages / sessions 表操作
BudgetGuard          ——  per-session / per-day token 上限
```
...
> **技术栈**：LangGraph.js + LangChain.js + Vue 3。
> 选这个组合的原因：已有框架基础、LangGraph 的 StateGraph 可视化是学习目标之一。
> **关键原则**：框架管图结构和 provider 胶水，核心业务逻辑（parseA2AMentions、assembleContext、BudgetGuard）仍自己写——不要把这些逻辑藏进黑盒。
...
| **Agent 编排** | **LangGraph.js `StateGraph`** | 把 ReAct loop 和跨 Cat handoff 各建一张图，状态流转可视化，断点调试方便——见 5.2 |
| **工具集成** | **`@langchain/core/tools` + `MemorySaver`** | tool() 定义省去手写 JSON Schema，MemorySaver 管 checkpoint；不用 AgentExecutor——见 5.2 |
| **LLM Provider** | **`@langchain/anthropic` / `@langchain/openai`** | 统一 `.stream()` 接口，换 provider 改一行——见 5.2 |
...
| **llm-call 重试** | LangChain 内置重试 + 统一 `callLLM()` 封装兜底 | 三家 API 各自的重试逻辑，LangChain 帮处理大部分——见 5.5 |
...
### 5.2 LangGraph + LangChain 的职责划分

框架管什么、自己写什么，是这套技术栈里最值得想清楚的问题。用错方向的后果：
- **框架管太多**：看不见 tool loop 怎么跑，A2A 路由也变黑盒，学不到原理
- **框架管太少**：等于没用框架，浪费了 StateGraph 的可视化和 provider 胶水

**正确的边界**：

```
LangGraph 管：图结构 + 状态流转可视化
  ├── ToolCallerGraph（内层）：单 Cat 的 ReAct tool loop
  │     节点：call_model → run_tools
  │     条件边：有 tool_use → run_tools；否则 → END
  │     这就是 LangGraph 文档里的标准 "Tool Node" 模式
  │
  └── MultiCatGraph（外层）：跨 Cat 的 Prompt-Chained Handoff
        节点：invoke_cat → route_next
        条件边：parseA2AMentions() 返回值决定跳哪个节点
        ⚠️ parseA2AMentions 自己实现，LangGraph 只管"按返回值跳"

LangChain 管：provider 适配 + 工具定义 + embedding
  ├── ChatAnthropic / ChatOpenAI / ChatGoogleGenerativeAI → 统一 .stream() 接口
  ├── tool() 帮助函数 → 省去手写 JSON Schema，自动做 zod 验证
  ├── MemorySaver → checkpointer，管 StateGraph 的中间状态持久化
  └── OpenAIEmbeddings / AnthropicEmbeddings → Phase 2 向量生成
  ❌ 不用：AgentExecutor / LLMChain / createReactAgent 默认实现
        （它们把 tool loop 黑盒化，看不见每一跳的 tool_use / tool_result）

我自己写：核心业务逻辑，不能进黑盒
  ├── parseA2AMentions()  — §3.4.3 的 9 条规则，任何框架都没有
  ├── MultiCatGraph 的条件边函数 routeToNextCat()
  ├── assembleContext()   — 三层记忆组装，MemorySaver 只管 checkpoint
  ├── SQLite CRUD         — notes / messages / sessions 表
  └── BudgetGuard         — per-session / per-day token 上限
```

#### ToolCallerGraph：单 Cat ReAct loop（LangGraph 实现）
...
// 工具定义：用 LangChain tool() 省去手写 JSON Schema
const searchNotesTool = tool(
  async ({ query }) => {
    const results = await db.searchNotesFTS(query, 5)
    return JSON.stringify(results)
  },
  { name: 'searchNotes', description: '在笔记库中搜索相关笔记', schema: z.object({ query: z.string() }) }
)

const saveNoteTool = tool(
  async ({ title, content, tags }) => {
    const note = await db.notes.insert({ title, content, tags, status: 'live' })
    return `已保存笔记 ${note.id}`
  },
  {
    name: 'saveNote',
    description: '把整理好的笔记存入知识库',
    schema: z.object({ title: z.string(), content: z.string(), tags: z.array(z.string()) })
  }
)

const tools = [searchNotesTool, saveNoteTool]
const toolNode = new ToolNode(tools)  // LangGraph 内置，自动路由 tool_use → 执行 → tool_result

const model = new ChatAnthropic({ model: 'claude-opus-4-8' }).bindTools(tools)

// 条件边：有 tool_use 就继续，否则结束
function shouldContinue(state: typeof MessagesAnnotation.State) {
  const last = state.messages.at(-1)!
  if ('tool_calls' in last && (last as any).tool_calls?.length > 0) return 'run_tools'
  return END
}

// 构建 ToolCallerGraph（这就是 §3.3 runToolLoop 的 LangGraph 版）
export const toolCallerGraph = new StateGraph(MessagesAnnotation)
  .addNode('call_model', async (state) => {
    // MAX_TOOL_LOOP_ITERATIONS 靠 recursionLimit 控制
    const response = await model.invoke(state.messages)
    return { messages: [response] }
  })
  .addNode('run_tools', toolNode)
  .addEdge('__start__', 'call_model')
  .addConditionalEdges('call_model', shouldContinue)
  .addEdge('run_tools', 'call_model')   // tool 结果回喂给模型
  .compile({ checkpointer: memorySaver, recursionLimit: 10 })  // recursionLimit = MAX_TOOL_LOOP_ITERATIONS
```

> **recursionLimit 就是 MAX_TOOL_LOOP_ITERATIONS**：LangGraph 用 `recursionLimit`（默认 25）来防死循环，传给 `.compile()` 或单次 `.invoke()` 都行。Phase 1 设 10 够用。

#### MultiCatGraph：跨 Cat Prompt-Chained Handoff（LangGraph 实现）

```typescript
import { Annotation, StateGraph, END } from '@langchain/langgraph'

// 跨 Cat 共享的图状态
const MultiCatState = Annotation.Root({
  messages:    Annotation<ChatMessage[]>({ reducer: (a, b) => [...a, ...b] }),
  currentCat:  Annotation<CatId>(),
  depth:       Annotation<number>({ default: () => 0 }),
  sessionId:   Annotation<string>(),
})

// 节点：调用当前 Cat（内部跑完整的 ToolCallerGraph）
async function invokeCat(state: typeof MultiCatState.State) {
  const context = await assembleContext(state.sessionId, lastUserMessage(state.messages))
  // 复用 ToolCallerGraph 作为子图（LangGraph 支持子图组合）
  const result = await toolCallerGraph.invoke(
    { messages: context },
    { configurable: { thread_id: `${state.sessionId}:${state.currentCat}` } }
  )
  const finalText = extractFinalText(result.messages)
  await db.messages.insert({ sessionId: state.sessionId, role: 'assistant', agentId: state.currentCat, content: finalText })
  return { messages: [{ role: 'assistant', content: finalText, agentId: state.currentCat }] }
}

// 条件边：parseA2AMentions 决定下一跳（这里是自己实现的核心逻辑）
function routeToNextCat(state: typeof MultiCatState.State): CatId | typeof END {
  if (state.depth >= MAX_A2A_DEPTH) return END
  const lastMsg = state.messages.at(-1)!
  const nextCats = parseA2AMentions(lastMsg.content, state.currentCat)  // 自己实现的 9 条规则
  if (nextCats.length === 0) return END
  return nextCats[0]  // Phase 1 只取第一个（串行）
}

// 构建 MultiCatGraph
const multiCatGraph = new StateGraph(MultiCatState)
  .addNode('claude', invokeCat)
  .addNode('gpt',    invokeCat)      // Phase 2 加
  .addNode('gemini', invokeCat)      // Phase 2 加
  .addEdge('__start__', 'claude')    // Phase 1 默认入口
  .addConditionalEdges('claude',  routeToNextCat, { claude: 'claude', gpt: 'gpt', gemini: 'gemini', [END]: END })
  .addConditionalEdges('gpt',     routeToNextCat, { claude: 'claude', gpt: 'gpt', gemini: 'gemini', [END]: END })
  .addConditionalEdges('gemini',  routeToNextCat, { claude: 'claude', gpt: 'gpt', gemini: 'gemini', [END]: END })
  .compile({ checkpointer: memorySaver })
```

> **关键认知不变**：`routeToNextCat` 里的决策是我的代码（`parseA2AMentions`），LangGraph 只是把"按返回值跳到哪个节点"这件事做成了图结构，让你能在 LangSmith / Studio 里看到每一跳的状态。这比手写 `while` 循环好调试——但原理没变。
...
> **版本对齐**：`@langchain/langgraph` 和 `@langchain/core` 必须版本匹配（见各自 `peerDependencies`），不对齐会有运行时类型错误。建议锁定版本，不要用 `^`。
...
W3 用 LangGraph 的 `ToolCallerGraph` 跑 ReAct loop，完整实现已在 §5.2。这里补充 W3 的关键接线点：

```typescript
// packages/api/src/routes/chat.ts
// SSE 端点：拿到用户输入 → assembleContext → 跑 ToolCallerGraph → 流式推给前端
fastify.get('/chat/stream', async (req, reply) => {
  reply.raw.setHeader('Content-Type', 'text/event-stream')
  reply.raw.setHeader('Cache-Control', 'no-cache')
  ...
  const ctx: RequestContext = {
    sessionId: req.headers['x-session-id'] as string ?? ulid(),
    promptVersion: 'capture@v3',
    platform: 'web',
  }

  // 1. 成本检查
  await assertWithinBudget(ctx.sessionId)

  // 2. 组装上下文（三层记忆）
  const messages = await assembleContext(ctx.sessionId, input)

  // 3. 跑 ToolCallerGraph（内层 ReAct loop），流式转发 chunk
  const stream = await toolCallerGraph.stream(
    { messages },
    {
      configurable: { thread_id: ctx.sessionId },
      recursionLimit: 10,     // = MAX_TOOL_LOOP_ITERATIONS
    }
  )

  for await (const chunk of stream) {
    reply.raw.write(`event: chunk\ndata: ${JSON.stringify(chunk)}\n\n`)
    // 同步写日志：token 计费、traceId 埋点（见 §5.5 callLLM）
  }
  reply.raw.end()
})
```

> **跟 routeSerial 的区别**：`routeSerial` / `MultiCatGraph` 是 Agent → Agent 的串行路由（A2A，Phase 2 才做），`ToolCallerGraph` 是 Agent → Tool → Agent 的工具循环（ReAct，Phase 1 就要跑通）。后者是前者的前置——一个 Agent 都没法自主调工具，谈不上多 Agent 自主协作。
...
*最后更新：2026-06-07 —— 框架选型改为 LangGraph + LangChain + Vue（§3.2 / §3.3 / §3.4 / §5 / §8.6 / 附录图 1 / 图 3 全部更新）；核心原则不变：框架管图结构和 provider 胶水，parseA2AMentions / assembleContext / BudgetGuard 仍自己写。*
...
- **框架选型：LangGraph + LangChain + Vue**（v2 最终决策）：ToolCallerGraph（内层 ReAct loop）+ MultiCatGraph（外层跨 Cat handoff）用 LangGraph StateGraph，provider 适配和工具定义用 LangChain（`ChatAnthropic` / `tool()` / `MemorySaver`）。核心业务逻辑（`parseA2AMentions` / `assembleContext` / `BudgetGuard` / SQLite CRUD）仍自己写，不进黑盒。见 §3.7 和 §5。
...
graph TB
    ...
    subgraph LangChainProviders["☁️ LangChain Provider 层"]
        ClaudeLC["ChatAnthropic\nPhase 1 唯一入口"]
        GPTLC["ChatOpenAI\nPhase 2 加入"]
        GeminiLC["ChatGoogleGenerativeAI\nPhase 2 加入"]
    end

    subgraph LLMAPIs["🌐 模型 API"]
    ...
    LLMCall --> ClaudeLC
    LLMCall -.->|"Phase 2"| GPTLC
    LLMCall -.->|"Phase 2"| GeminiLC
    ...
    style GPTLC stroke-dasharray: 5 5
    style GeminiLC stroke-dasharray: 5 5
```

> 虚线 = Phase 2 才启用；实线 = Phase 1 必须实现。LangChain Provider 层统一封装 `.stream()` 接口，换 provider 只改一行。
...
flowchart TD
    ...
    Router["MultiCatGraph（LangGraph StateGraph）\n__start__ → claude\ndepth: 0"]

    Router -->|"invoke_cat 节点"| Claude

    subgraph Claude["🐱 claude 节点（一回合）"]
        CL_Loop["ToolCallerGraph 子图\n（内层 ReAct，见图 2）"]
        CL_Text["finalText\n= 完整回复文本"]
        CL_Loop --> CL_Text
    end
    ...
    Parse1["routeToNextCat 条件边\n= parseA2AMentions(finalText, 'claude')\n规则 1–9：剥代码块 + 行首 + markdown前缀\n+ token边界 + 长匹配 + 空白容忍 + 过滤自调用"]

    Parse1 -->|"返回 END（无 @mention）"| End1([结束 ✓])
    Parse1 -->|"返回 'gpt'"| GPT

    subgraph GPT["🤖 gpt 节点（一回合）"]
        GP_Loop["ToolCallerGraph 子图"]
        GP_Text["finalText"]
        GP_Loop --> GP_Text
    end

    GP_Text --> Parse2["routeToNextCat 条件边\nparseA2AMentions(finalText, 'gpt')"]
    Parse2 -->|"返回 END"| End2([结束 ✓])
    Parse2 -->|"返回 'claude'"| Claude2["...继续链条..."]

    Claude2 --> CheckDepth2{"MultiCatState.depth\n>= MAX_A2A_DEPTH=15?"}
    CheckDepth2 -->|"否"| Claude
    CheckDepth2 -->|"是（depth=15）"| End3([截断 ⚠️])
