...
Phase 2 核心目标达成：A2A 跨 Agent 接力跑通，context-transport 上线替换硬截断，模型按场景分流落地。9/10 项完成，verdict-detect 是唯一遗留债。
...
| 9 | 模型按场景分流（resolve_model + GPT/Gemini provider） | ✅ | 本 commit |
...
|------|-------------|---------|--------|
| verdict-detect | `verdict-detect.ts` | 链路终止判定，靠 `recursion_limit=10` 兜底 | 🟡 |
...
1. **verdict-detect** — 链路终止判定，防止 Agent 互踢皮球。当前 `recursion_limit=10` 是硬兜底，应该在 LLM 层面判断"这个回答已经够了，不需要再 @其他 Agent"
2. **evals 升级** — Phase 1 黄金集只测单 Agent tool call，Phase 3 需要 A2A 端到端 evals（knowledge → review 链路至少 5 条）
3. **MultiMentionOrchestrator** — 支持一个用户消息同时 @多个 Agent 并行执行
4. **Gemini 接入** — 联想/头脑风暴场景，前提是黄金集人评出现 ≥5 条"需要联想视角"的样本
5. **a2a-shadow-detection** — 用 AST/更多正则覆盖行内 @mention
